...
opt = Adam(learning_rate=INIT_LR)  # Fix: use 'learning_rate' instead of 'lr'
...
print(classification_report(
    ...
    predictions.argmax(axis=1),  # Convert predicted one-hot encoded labels back to integers
    target_names=[str(x) for x in le.classes_]  # Get the label names (e.g., '0', '1', ...)
))
