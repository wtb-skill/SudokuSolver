...
class RecognitionAccuracyEvaluator:
    ...
    def _setup_logging(self):
        logging.getLogger(
            "modules.sudoku_image_pipeline.step_2_board_detector"
    ...
    def evaluate(self):
        ...
        with tqdm(
            total=len(files), desc="Evaluating Sudoku Images", unit="image"
        ) as pbar:
        ...
        overall_accuracy = (
            round(100 * self.correct_digits / self.total_digits, 2)
            if self.total_digits
            else 0
        )
...
if __name__ == "__main__":
    ...
    for model in model_files:
        shutil.move(
            os.path.join(models_path, model), os.path.join(current_model_path, model)
        )
        evaluator = RecognitionAccuracyEvaluator()
        evaluator.model_name = model
        evaluator.evaluate()
        shutil.move(
            os.path.join(current_model_path, model),
            os.path.join(all_versions_path, model),
        )
