...
class DeterministicDigitDatasetGenerator:
    ...
    def create_digit_image(self, digit: int, font_path: str, font_size: int, shift_x: int, shift_y: int) -> np.ndarray:
    ...
    def apply_rotation(self, img: np.ndarray, angle: int) -> np.ndarray:
        matrix = cv2.getRotationMatrix2D((self.image_size / 2, self.image_size / 2), angle, 1)
    ...
    def generate_images(self):
        ...
        total_images = len(self.font_paths) * len(self.font_sizes) * len(self.blur_levels) * \
                       len(self.shift_ranges) * len(self.rotations) * len(self.noise_levels) * 9
                                ...
                                for shift_y in self.shift_ranges:
                                    for rotation in self.rotations:
                                        for noise_level in self.noise_levels:
                                            img = self.create_digit_image(digit, font_path, font_size, shift_x, shift_y)
                                            ...
                                            filename = f"{os.path.basename(font_path)}_{font_size}_" \
